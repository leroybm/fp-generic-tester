...
import { AdOptions, ExtendedFluidPlayerOptions } from "../../models/ConfiguratorOptions";
...
describe("AdvertismentForm", () => {
  ...
  it('should fill form for midRoll', async () => {
    const user = userEvent.setup();
    const adList = [{ _id: uniqueId(), roll: 'midRoll', vastTag: 'https://' }] as AdOptions[];
    // @ts-expect-error
    render(<DummyComponent defaultValues={{ vastOptions: { adList } }} />)
  ...
  it('should change displayed form', async () => {
    const user = userEvent.setup();
    const adList = [{ _id: uniqueId(), roll: 'preRoll', vastTag: 'https://' }] as AdOptions[];
    // @ts-expect-error
    render(<DummyComponent defaultValues={{ vastOptions: { adList } }} />)
  ...
  it('should update with every entry', async () => {
    let lastUpdateValue = {} as AdOptions;
  ...
  it('should show validate Timer', async () => {
    const user = userEvent.setup();
    const adList = [{ _id: uniqueId(), roll: 'midRoll', vastTag: '' }] as AdOptions[];
    // @ts-expect-error
    render(<DummyComponent defaultValues={{ vastOptions: { adList } }} />)
